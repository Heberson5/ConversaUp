...
function createConnection(connId, name, color) {
  // Se já existir um cliente para esta conexão, destruí-lo antes de criar outro
  ...
  client.on('disconnected', async (reason) => {
    ...
    // Tenta reconectar automaticamente se estiver ativa e não for logout proposital
  ...
  client.on("message_ack", (msg, ack) => {
    ...
    io.emit("message_ack_update", { id: msg.id._serialized, chatId: msg.fromMe ? msg.to : msg.from, ack });
  });
  ...
  client.on("message", async (msg) => {
    ...
    try {
      ...
      // Lógica de pesquisa (só para legado, mantido)
  ...
  socket.on("send_message", async (data, callback) => {
    ...
    try {
      ...
      if (file) {
        ...
        await client.sendMessage(destinatario, media, { sendAudioAsVoice: isPtt || false });
      } else {
        await client.sendMessage(destinatario, text, { quotedMessageId });
      }
...
socket.on("get_contacts", async () => {
  let allContacts = [];
  // Itera sobre todas as conexões ativas (enabled e connected)
  for (const conn of connections) {
    if (!conn.enabled || conn.status !== 'connected' || !conn.client) continue;
    try {
      const contacts = await conn.client.getContacts();
      const formatted = contacts
        .filter(c => c.isUser && c.id._serialized) // apenas contatos de usuário (não grupos, não broadcasts)
        .map(c => ({
          id: c.id._serialized,
  ...
  // Remove duplicatas (mesmo contato em múltiplas conexões? raro, mas pode acontecer)
