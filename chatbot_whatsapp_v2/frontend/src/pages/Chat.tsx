...
const Chat = () => {
  ...
  // Função para sanitizar mensagem antes de salvar (remover body de qualquer mídia)
  ...
  // ==========================================================================
  // Carregar mensagens do localStorage para todos os chats sempre que a lista mudar
  // ==========================================================================
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleMessageAck = (data: { id: string; chatId: string; ack: number }) => {
      if (selectedContactRef.current && data.chatId === selectedContactRef.current.id) {
        const updatedMessages = chatMessages.map(msg => msg.id === data.id ? { ...msg, ack: data.ack } : msg);
  ...
  // ==========================================================================
  // CORREÇÃO: ENVIO DE ÁUDIO (usando audio/webm)
  // ==========================================================================
  const startRecording = async () => {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      setRecordingStream(stream);
      // Especifica o tipo MIME para garantir compatibilidade
  ...
  // ==========================================================================
  // CORREÇÃO: ENVIO DE CONTATO (como mensagem, não nova conversa)
  // ==========================================================================
  const handleSendContact = (contact: { name: string; number: string; id: string }) => {
    if (!selectedContact) return;
    // Emite o contato como mensagem para a conversa atual
    socket.emit("send_message", { 
      to: selectedContact.id, 
      type: 'contact', 
    ...
    // Adiciona mensagem localmente para feedback visual
    ...
    setChatMessages(prev => [...prev, tempMsg]);
    if (messagesCache.current[selectedContact.id]) {
      messagesCache.current[selectedContact.id].push(tempMsg);
      saveToLocalStorage(selectedContact.id, selectedContact.connectionId, messagesCache.current[selectedContact.id]);
    }
  ...
  // ==========================================================================
  // Fechamento de modais ao clicar fora (corrigido)
  // ==========================================================================
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      const target = e.target as Element;
      // Fecha anexos se o clique não for no botão de anexo nem no conteúdo do anexo
      if (isAttachmentOpen && !target.closest('.attachment-container') && !target.closest('.attachment-picker-container')) {
        setIsAttachmentOpen(false);
      }
      // Fecha emoji picker se o clique não for no botão de emoji nem no picker
      if (isEmojiOpen && !target.closest('.emoji-container') && !target.closest('.emoji-picker-container')) {
        setIsEmojiOpen(false);
      }
      // Fecha quick replies se o clique não for no botão de zap nem no container
  ...
  // ==========================================================================
  const activeConnectionIds = Object.entries(connectionStatuses)
    .filter(([_, data]) => data.enabled && data.status === 'connected')
    .map(([id]) => id);
