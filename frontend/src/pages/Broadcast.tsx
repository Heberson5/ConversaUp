...
class BroadcastEngine {
  ...
  sendMode: 'safe' | 'unsafe' | null = null; // MODO SEGURO / INSEGURO
  ...
  saveState() {
    if (typeof window !== 'undefined') {
      localStorage.setItem('zapflow_engine_state', JSON.stringify({
  ...
  loadState() {
    if (typeof window !== 'undefined') {
      ...
      if (saved) {
        ...
        this.numberSuffix = data.numberSuffix !== undefined ? data.numberSuffix : '@s.whatsapp.net';
  ...
  checkIfAllowedTime(): boolean {
    ...
    // Checa agendamento inicial
    if (this.scheduleStart) {
      if (now < new Date(this.scheduleStart)) return false;
    }
    
    // Checa limite final
    if (this.scheduleEnd) {
      if (now > new Date(this.scheduleEnd)) {
        this.status = 'idle'; 
        this.isWaitingSchedule = false;
        return false;
      }
    ...
    if (hm < wh.start || hm > wh.end) return false;
  ...
  async start() {
    ...
    // LÓGICA DO MODO SEGURO: OBTER LISTA DE CHATS EXISTENTES
    ...
    for (let i = this.currentIndex; i < this.contacts.length; i++) {
      ...
      if (i > 0 && i % 80 === 0) {
        const batchMin = 180; 
        const batchMax = 360; 
        const batchDelayInSeconds = Math.floor(Math.random() * (batchMax - batchMin + 1)) + batchMin;
        
        console.log(`⏱️ Lote de 80 envios atingido. Pausa longa de ${batchDelayInSeconds} segundos...`);
        this.isBatchPausing = true;
        this.notify();
        
        for (let ms = 0; ms < batchDelayInSeconds * 10; ms++) {
          if (this.status === 'idle') break;
          while (this.status === 'paused') await new Promise(res => setTimeout(res, 500));
      ...
      const min = Number(this.minInterval) || 5;
      const max = Number(this.maxInterval) || 15;
      const delayInSeconds = Math.floor(Math.random() * (max - min + 1)) + min;
      
      console.log(`⏱️ Aguardando intervalo de segurança: ${delayInSeconds} segundos...`);
      
      for (let ms = 0; ms < delayInSeconds * 10; ms++) {
        if (this.status === 'idle') break;
        while (this.status === 'paused') await new Promise(res => setTimeout(res, 500));
        await new Promise(res => setTimeout(res, 100));
      }
      ...
      // ==========================================================
      // APLICAÇÃO DA REGRA DE MODO SEGURO
      // ==========================================================
      if (this.sendMode === 'safe') {
      ...
      if (this.socket && this.socket.connected) {
        try {
          if (this.mediaBase64 && this.mediaFile) {
            ...
            if (isAudio) {
              if (finalMsg.trim()) {
                await this.sendWithAck({ to: formattedTarget, text: finalMsg });
              ...
              const response = await this.sendWithAck(audioPayload);
              if (response.error || response.success === false || response.status === 'error' || response.status === 'ERROR') {
                throw new Error(response.error || 'Erro no servidor/API ao enviar áudio');
              }
            } else {
              const fullPayload = {
                to: formattedTarget,
              ...
              const response = await this.sendWithAck(fullPayload);
              if (response.error || response.success === false || response.status === 'error' || response.status === 'ERROR') {
                throw new Error(response.error || 'Erro no servidor/API ao enviar mensagem');
              }
            }
          } else {
            const response = await this.sendWithAck({ to: formattedTarget, text: finalMsg });
            if (response.error || response.success === false || response.status === 'error' || response.status === 'ERROR') {
              throw new Error(response.error || 'Erro no servidor/API ao enviar mensagem');
            }
          }
...
const Broadcast = () => {
  ...
  // Estado para Área Colapsável de Agendamento
  const [isScheduleOpen, setIsScheduleOpen] = useState(false);
  
  // Estado para o Picker de Emoji
  const [isEmojiOpen, setIsEmojiOpen] = useState(false);

  // Estados para o Modal de Aviso de Risco
  ...
  const emojiCategories = {
    ...
    "Animais": ["🐶","🐱","🐭","🐹","🐰","🦊","🐻","🐼","🐨","🐯","🦁","🐮","🐷","🐸","🐵","🐔","🐧","🐦","🐤","🦆","🦅","🦉","🦇","🐺","🐗","🐴","🦄","🐝","🐛","🦋","🐌","🐞","🐜","🦟","🦗","🕷","🦂","🐢","🐍","🦎","🦖","🦕","🐙","🦑","🦐","🦞","🦀","🐡","🐠","🐟","🐬","🐳","🐋","🦈","🐊","🐅","🐆","🦓","🦍","🐘","🦛","🦏","🐪","🐫","🦒","🦘","🐃","🐂","🐄","🐎","🐖","🐏","🐑","🦙","🐐","🦌","🐕","🐩","🐈","🐓","🦃","🦚","🦜","🦢","🦩","🕊","🐇","🦝","🦨","🦡","🦦","🦥","🐁","🐀","🐿","🦔","🐾","🐉","🐲","🌵","🎄","🌲","🌳","🌴","🪵","🌱","🌿","☘️","🍀","🎍","🪴","🎋","🍃","🍂","🍁","🍄","🐚","🌾","💐","🌷","🌹","🥀","🌺","🌸","🌼","🌻"],
    "Comida": ["🍏","🍎","🍐","🍊","🍋","🍌","🍉","🍇","🍓","🍈","🍒","🍑","🥭","🍍","🥥","🥝","🍅","🍆","🥑","🥦","🥬","🥒","🌶","🌽","🥕","🧄","🧅","🥔","🍠","🥐","🥯","🍞","🥖","🥨","🧀","🥚","🍳","🧈","🥞","🧇","🥓","🥩","🍗","🍖","🌭","🍔","🍟","🍕","🥪","🥙","🌮","🌯","🥗","🥘","🥫","🍝","🍜","🍲","🍛","🍣","🍱","🥟","🦪","🍤","🍙","🍚","🍘","🍥","🥠","🥮","🍢","🍡","🍧","🍨","🍦","🥧","🧁","🍰","🎂","🍮","🍭","🍬","🍫","🍿","🍩","🍪","🌰","🥜","🍯","🥛","🍼","☕️","🍵","🧃","🥤","🍶","🍺","🍻","🥂","🍷","🥃","🍸","🍹","🧉","🍾","🧊","🥄","🍴","🍽","🥣","🥡","🥢","🧂"],
    "Atividades": ["⚽️","🏀","🏈","⚾️","🥎","🎾","🏐","🏉","🥏","🎱","🪀","🏓","🏸","🏒","🏑","🥍","🏏","🪃","🥅","⛳️","🪁","🏹","🎣","🤿","🥊","🥋","🎽","🛹","🛼","🛷","⛸","🥌","🎿","⛷","🏂","🪂","🏋️‍♀️","🏋️","🤺","🤼‍♀️","🤼","🤸‍♀️","🤸","⛹️‍♀️","⛹️","🤾‍♀️","🤾","🏌️‍♀️","🏌️","🏇","🧘‍♀️","🧘","🏄‍♀️","🏄","🏊‍♀️","🏊","🤽‍♀️","🤽","🚣‍♀️","🚣","🧗‍♀️","🧗","🚵‍♀️","🚵","🚴‍♀️","🚴","🏆","🥇","🥈","🥉","🏅","🎖","🏵","🎗","🎫","🎟","🎪","🤹","🎭","🩰","🎨","🎬","🎤","🎧","🎼","🎹","🥁","🪘","🎷","🎺","🪗","🎸","🪕","🎻","🎲","♟","🎯","🎳","🎮","🎰","🧩"]
  };

  useEffect(() => {
    // === LÓGICA DE TEMA AUTOMÁTICO FORÇADO NO ROOT ===
    ...
    // Aplica no momento em que carrega a página
    handleThemeChange(mediaQuery);
    // Fica escutando caso o usuário mude o tema do SO com a aba aberta
    mediaQuery.addEventListener('change', handleThemeChange);

    const todayStr = new Date().toLocaleDateString('pt-BR');
    const savedWarningDate = localStorage.getItem('zapflow_broadcast_warning_date');
    if (savedWarningDate !== todayStr) {
  ...
  const handleRemoveDuplicates = () => {
    ...
    if (removedCount > 0) {
      toast.success(`${removedCount} contatos duplicados foram removidos!`);
    } else {
      toast.info("A fila já está limpa. Nenhum contacto duplicado encontrado.");
    }
  };
  ...
  // ============================================================================
  // FUNÇÕES DE EXPORTAÇÃO (EXCEL & PDF) CORRIGIDAS
  // ============================================================================
  const exportToExcel = () => {
  ...
  const exportToPDF = () => {
    ...
    // Cálculo dos dados do Gráfico de Pizza
    ...
    const html = `
      ...
      <head>
        ...
        <style>
          ...
          /* Estrutura do Dashboard (Info + Gráfico) */
          ...
          /* Gráfico de Pizza via CSS e Legendas */
          ...
          <div class="chart-card">
            ...
            <div class="legend">
               <div class="legend-item">
                  <span class="legend-label"><div class="dot success"></div> Enviados</span>
                  <span><strong>${successPct}%</strong> (${successCount})</span>
               </div>
               <div class="legend-item">
                  <span class="legend-label"><div class="dot fail"></div> Falhas</span>
                  <span><strong>${failPct}%</strong> (${failCount})</span>
               </div>
            </div>
          </div>
        </div>
  ...
  return (
    <AppLayout>
      {/* Wrapper de Preenchimento Completo (Cobre a tela inteira garantindo a harmonia do tema) */}
      <div className="min-h-[calc(100vh-4rem)] w-full bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
        
        {/* MODAL OBRIGATÓRIO DE AVISO DE RESPONSABILIDADE */}
        {showWarningModal && (
            ...
            <div className="bg-white dark:bg-slate-900 max-w-lg w-full rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-slate-100 dark:border-slate-800">
              ...
              <div className="p-6 space-y-4 text-slate-600 dark:text-slate-400">
                <p className="font-medium text-slate-800 dark:text-slate-200">
                  O envio de mensagens em massa não solicitadas (SPAM) fere diretamente as políticas e Termos de Serviço do WhatsApp.
                </p>
                <p className="text-sm">
                  Nós <strong className="text-rose-600 dark:text-rose-400 underline">não nos responsabilizamos</strong> por eventuais bloqueios, restrições ou o banimento definitivo do seu número de WhatsApp. O uso desta ferramenta de disparos é de sua <strong className="text-slate-800 dark:text-slate-200">inteira responsabilidade</strong>.
                </p>
                ...
                <label className="flex items-center gap-3 p-3 mt-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition border border-slate-200 dark:border-slate-700/50">
                  <input 
                    ...
                    className="w-5 h-5 rounded text-rose-500 focus:ring-rose-500 border-slate-300 dark:border-slate-600 dark:bg-slate-900"
                  />
              ...
              <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex justify-end">
                <button 
                  onClick={() => {
                    if (dontShowWarningToday) {
                      localStorage.setItem('zapflow_broadcast_warning_date', new Date().toLocaleDateString('pt-BR'));
                    }
                    setShowWarningModal(false);
                  }} 
                  className="px-6 py-3 bg-rose-500 text-white rounded-xl font-bold hover:bg-rose-600 transition shadow-lg shadow-rose-500/20 w-full"
                >
                  Eu entendi e assumo a responsabilidade
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="p-4 lg:p-8 max-w-6xl mx-auto pb-24 text-slate-900 dark:text-slate-100">
          ...
          {/* BANNER FIXO E VERMELHO NO TOPO DA TELA APÓS SAIR DO MODAL */}
          {!showWarningModal && (
          ...
          <div className="mb-8 flex justify-between items-start">
            ...
            <div className="flex gap-4">
              ...
              {/* BOTÃO EXCLUSIVO PARA ADMINISTRADOR MASTER */}
              {isMaster && (
                <button 
                  onClick={handleResetDailyLimit}
                  disabled={isBusy}
                  className={cn(
                    "px-4 py-2 bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 rounded-xl border border-rose-200 dark:border-rose-500/20 shadow-sm text-xs font-bold flex items-center gap-2 transition",
                    isBusy ? "opacity-50 cursor-not-allowed" : "hover:bg-rose-100 dark:hover:bg-rose-500/20"
                  )}
                >
              ...
              <button 
                ...
                disabled={isBusy}
                className={cn(
                  "px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm text-xs font-bold flex items-center gap-2 transition",
                  isBusy ? "opacity-50 cursor-not-allowed" : "hover:bg-slate-200 dark:hover:bg-slate-700"
                )}
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {/* LADO ESQUERDO: CONTEÚDO, CONFIGS, EXPEDIENTE */}
            <div className="lg:col-span-2 space-y-6">
              
              {/* SESSÃO 1: CONTEÚDO DA MENSAGEM */}
              <div className={cn("bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all", isBusy && "opacity-60 pointer-events-none")}>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {/* Botões de Variáveis Dinâmicas (Spintax) */}
                ...
                <div className="w-full border border-slate-200 dark:border-slate-800 rounded-xl focus-within:ring-2 focus-within:ring-emerald-500/50 bg-slate-50 dark:bg-slate-950 relative flex flex-col mb-4 transition-all">
                  <textarea 
                    ...
                    onChange={(e) => { engine.message = e.target.value; engine.notify(); handleTextareaActivity(); }}
                    onBlur={handleTextareaActivity}
                    onClick={handleTextareaActivity}
                    onKeyUp={handleTextareaActivity}
                    placeholder="Olá {{Nome}}, tudo bem? Escreva sua mensagem aqui..."
                    className="w-full p-4 bg-transparent text-slate-800 dark:text-slate-200 text-sm min-h-[250px] outline-none resize-y placeholder-slate-400 dark:placeholder-slate-600"
                  />
                  ...
                  <div className="flex items-center justify-between px-2 py-2 border-t border-slate-200/60 dark:border-slate-800/60 bg-white/50 dark:bg-slate-900/50 rounded-b-xl">
                    ...
                    <div className="flex items-center gap-1">
                       <button onClick={() => applyFormatting('*', '*')} className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-800 rounded text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition" title="Negrito"><Bold size={16}/></button>
                       <button onClick={() => applyFormatting('_', '_')} className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-800 rounded text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition" title="Itálico"><Italic size={16}/></button>
                       <button onClick={() => applyFormatting('~', '~')} className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-800 rounded text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition" title="Tachado"><Strikethrough size={16}/></button>
                       <button onClick={() => applyFormatting('\u0060\u0060\u0060', '\u0060\u0060\u0060')} className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-800 rounded text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition" title="Código Monoespaçado"><Code size={16}/></button>
                       <div className="w-[1px] h-4 bg-slate-300 dark:bg-slate-700 mx-1"></div>
                       <button onClick={() => applyFormatting('> ', '')} className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-800 rounded text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition" title="Citação"><Quote size={16}/></button>
                       <button onClick={() => applyFormatting('- ', '')} className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-800 rounded text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition" title="Lista"><ListIcon size={16}/></button>
                    </div>

                    {/* Emoji */}
                    <div className="relative emoji-container">
                      <button onClick={() => setIsEmojiOpen(!isEmojiOpen)} className="p-2 text-slate-500 dark:text-slate-400 hover:text-emerald-500 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 rounded-lg transition">
                        <Smile size={20} />
                      </button>
                             ...
                             {Object.entries(emojiCategories).map(([category, emojis]) => (
                               <div key={category} id={`emoji-cat-${category}`} className="mb-4">
                                 ...
                                 <div className="grid grid-cols-7 gap-1">
                                   {emojis.map((emoji, i) => (
                                     <button key={i} onClick={() => insertText(emoji)} className="text-xl hover:bg-slate-200/80 dark:hover:bg-slate-700/80 rounded transition p-1 flex items-center justify-center">
                                       {emoji}
                                     </button>
                                   ))}
                                 </div>
              ...
              {/* SESSÃO 2: CONFIGURAÇÕES E INTERVALO */}
              <div className={cn("bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all", isBusy && "opacity-60 pointer-events-none")}>
                <h3 className="font-bold text-slate-800 dark:text-slate-200 mb-4 flex items-center gap-2">
                  <Settings size={18} className="text-amber-500" />
                  Configurações & Intervalo
                </h3>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase block mb-2">Pausa Mínima (seg)</label>
                    <input type="number" value={engine.minInterval} onChange={(e) => { engine.minInterval = parseInt(e.target.value) || 0; engine.notify(); }} className="w-full p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-600 dark:text-slate-300 outline-none focus:ring-2 focus:ring-emerald-500" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase block mb-2">Pausa Máxima (seg)</label>
                    <input type="number" value={engine.maxInterval} onChange={(e) => { engine.maxInterval = parseInt(e.target.value) || 0; engine.notify(); }} className="w-full p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-600 dark:text-slate-300 outline-none focus:ring-2 focus:ring-emerald-500" />
                  </div>
                </div>
              </div>

              {/* SESSÃO NOVA: AGENDAMENTO E EXPEDIENTE (COLAPSÁVEL) */}
              <div className={cn("bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all overflow-hidden", isBusy && "opacity-60 pointer-events-none")}>
                <button 
                  ...
                  className="w-full p-6 font-bold text-slate-800 dark:text-slate-200 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"
                >
                ...
                {isScheduleOpen && (
                  <div className="px-6 pb-6 animate-in slide-in-from-top-4 duration-300">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 border-b border-slate-100 dark:border-slate-800 pb-6 pt-2">
                      <div>
                        <label className="text-[10px] font-bold text-slate-400 uppercase block mb-2">Agendar Início (Opcional)</label>
                        <input type="datetime-local" value={engine.scheduleStart} onChange={e => { engine.scheduleStart = e.target.value; engine.notify(); }} className="w-full p-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs outline-none focus:ring-2 focus:ring-indigo-500 text-slate-600 dark:text-slate-300" />
                      </div>
                      <div>
                        <label className="text-[10px] font-bold text-slate-400 uppercase block mb-2">Agendar Fim / Parada (Opcional)</label>
                        <input type="datetime-local" value={engine.scheduleEnd} onChange={e => { engine.scheduleEnd = e.target.value; engine.notify(); }} className="w-full p-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs outline-none focus:ring-2 focus:ring-rose-500 text-slate-600 dark:text-slate-300" />
                      </div>
                    </div>
                          ...
                          <div key={day} className="flex items-center justify-between p-2 bg-slate-50 dark:bg-slate-950 rounded-lg border border-slate-100 dark:border-slate-800 hover:border-indigo-100 dark:hover:border-indigo-500/30 transition-all">
                            <div className="flex items-center gap-2 w-1/3">
                              <input 
                                ...
                                onChange={e => { engine.workingHours[day as keyof typeof engine.workingHours].enabled = e.target.checked; engine.notify(); }} 
                                className="w-4 h-4 rounded text-indigo-500 cursor-pointer dark:bg-slate-800 dark:border-slate-600" 
                              />
                              <span className={cn("text-xs font-bold", config.enabled ? "text-slate-700 dark:text-slate-300" : "text-slate-400 dark:text-slate-600 line-through")}>
                                {daysOfWeek[day]}
                              </span>
                            </div>
                            <div className="flex items-center gap-2 w-2/3 justify-end">
                              <input 
                                ...
                                value={config.start} 
                                onChange={e => { engine.workingHours[day as keyof typeof engine.workingHours].start = e.target.value; engine.notify(); }} 
                                className="p-1 border border-slate-200 dark:border-slate-700 rounded text-xs bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 disabled:opacity-50 disabled:bg-slate-100 dark:disabled:bg-slate-800 outline-none" 
                              />
                              ...
                              <input 
                                ...
                                disabled={!config.enabled} 
                                value={config.end} 
                                onChange={e => { engine.workingHours[day as keyof typeof engine.workingHours].end = e.target.value; engine.notify(); }} 
                                className="p-1 border border-slate-200 dark:border-slate-700 rounded text-xs bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 disabled:opacity-50 disabled:bg-slate-100 dark:disabled:bg-slate-800 outline-none" 
                              />
                            </div>
                          </div>
            ...
            {/* LADO DIREITO: MONITOR, DESTINATÁRIOS E LOGS */}
            <div className="space-y-6 lg:sticky lg:top-4 lg:self-start">
              
              {/* SESSÃO 4: MONITOR DE ENVIO (Painel agora é Dinâmico) */}
              <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-6 rounded-2xl shadow-sm text-slate-900 dark:text-white transition-all">
                ...
                {/* Alerta de Aguardo de Expediente */}
                ...
                <div className="space-y-4">
                  ...
                  {/* BOTÕES DE INICIAR / MODO DE DISPARO */}
                      ...
                      <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                        ...
                        <div className="grid grid-cols-1 gap-2">
                          ...
                          >
                            ...
                            <div>
                              <span className="font-bold text-sm block">Modo Seguro</span>
                              <span className="text-[10px] leading-tight block mt-0.5 opacity-80">Apenas para contatos que você já conversou. Ignora números novos para evitar bloqueios.</span>
                            </div>
                          </button>
                          
                          <button 
                            onClick={() => { engine.sendMode = 'unsafe'; engine.notify(); }}
                            className={cn(
                              "p-3 rounded-xl border text-left transition-all flex items-start gap-3",
                              engine.sendMode === 'unsafe' 
                                ? "bg-amber-50 dark:bg-amber-500/20 border-amber-500 text-amber-600 dark:text-amber-400" 
                                : "bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700"
                            )}
                          >
                            <ShieldAlert size={20} className="shrink-0 mt-0.5" />
                            <div>
                              <span className="font-bold text-sm block">Modo Inseguro</span>
                              <span className="text-[10px] leading-tight block mt-0.5 opacity-80">Envia para todos os contatos da lista, independente de histórico (Maior risco de banimento).</span>
                            </div>
                          </button>
                        </div>
                      </div>
                  ...
                  {/* BOTÕES DURANTE EXECUÇÃO */}
                  {(engine.status === 'sending' || engine.status === 'paused') && (
                    <div className="space-y-4">
                      {/* Exibe o Modo Ativo, porém bloqueado e inerte */}
                      <div className={cn(
                        "p-3 rounded-xl border flex items-start gap-3 opacity-70 pointer-events-none",
                        engine.sendMode === 'safe' 
                          ? "bg-emerald-50 dark:bg-emerald-500/10 border-emerald-500/50 text-emerald-600 dark:text-emerald-400" 
                          : "bg-amber-50 dark:bg-amber-500/10 border-amber-500/50 text-amber-600 dark:text-amber-400"
                      )}>
                        {engine.sendMode === 'safe' ? <ShieldCheck size={20} className="shrink-0 mt-0.5" /> : <ShieldAlert size={20} className="shrink-0 mt-0.5" />} 
                        <div>
                          <span className="font-bold text-sm block">{engine.sendMode === 'safe' ? 'Modo Seguro Ativo' : 'Modo Inseguro Ativo'}</span>
                          <span className="text-[10px] leading-tight block mt-0.5">
                            {engine.sendMode === 'safe' 
                              ? 'Ignorando números novos sem histórico de conversa.' 
                              : 'Enviando para todos os números da lista importada.'}
                          </span>
                        </div>
                      </div>

                      <div className="grid grid-cols-2 gap-2">
                        {engine.status === 'sending' ? (
                          <button onClick={() => engine.pause()} className="w-full py-3 bg-amber-500 hover:bg-amber-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg shadow-amber-500/20">
                            <Pause size={18} /> Pausar
                          </button>
                        ) : (
                          <button onClick={() => engine.resume()} className="w-full py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg shadow-blue-500/20">
                            <Play size={18} /> Continuar
                          </button>
                        )}
                        
                        <button onClick={() => engine.stop()} className="w-full py-3 bg-rose-500 hover:bg-rose-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg shadow-rose-500/20">
              ...
              {/* SESSÃO 3: DESTINATÁRIOS (MEIO DIREITO) */}
              <div className={cn("bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all", isBusy && "opacity-60 pointer-events-none")}>
                <h3 className="font-bold text-slate-800 dark:text-slate-200 mb-4 flex items-center gap-2">
                      ...
                      {manualContacts.map((contact, idx) => (
                        <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                          <div className="col-span-6">
                            <input placeholder="Número" value={contact.number} onChange={(e) => updateManualContact(idx, 'number', e.target.value)} className="w-full p-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-xs text-slate-600 dark:text-slate-300 outline-none focus:ring-1 focus:ring-blue-500" />
                          </div>
                          <div className="col-span-5">
                            <input placeholder="Nome" value={contact.name} onChange={(e) => updateManualContact(idx, 'name', e.target.value)} className="w-full p-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-xs text-slate-600 dark:text-slate-300 outline-none focus:ring-1 focus:ring-blue-500" />
                          </div>
                          <button onClick={() => removeManualRow(idx)} className="col-span-1 text-slate-300 dark:text-slate-600 hover:text-rose-500 dark:hover:text-rose-400 transition-colors flex justify-center">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      ))}
                    </div>
                    <button onClick={addManualRow} className="w-full py-2 border-2 border-dashed border-slate-200 dark:border-slate-700 text-slate-400 rounded-xl text-[10px] font-bold hover:bg-slate-50 dark:hover:bg-slate-800/50 flex items-center justify-center gap-2">
                      <Plus size={14} /> Adicionar Linha
                    </button>
                    <button onClick={handleManualImport} className="w-full py-3 bg-blue-500 text-white rounded-xl text-xs font-bold hover:bg-blue-600 transition-all shadow-md shadow-blue-500/20">
                      Confirmar Lista
                    </button>
                  </TabsContent>

                  <TabsContent value="file">
                    <button onClick={() => fileInputRef.current?.click()} className="w-full py-12 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl flex flex-col items-center justify-center gap-3 text-slate-400 dark:text-slate-500 hover:text-emerald-600 dark:hover:text-emerald-400 hover:border-emerald-200 hover:bg-emerald-50/50 dark:hover:bg-emerald-500/10 transition-all">
                      <FileUp size={36} />
                      <span className="text-sm font-bold">Importar .xlsx ou .csv</span>
                      <span className="text-[10px] text-slate-400 dark:text-slate-500 text-center max-w-[200px]">A primeira coluna deve conter os números de telefone</span>
                    </button>
                ...
                {total > 0 && (
                  <div className="mt-4 p-4 bg-emerald-50 dark:bg-emerald-500/10 rounded-xl border border-emerald-100 dark:border-emerald-500/20 flex flex-col gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-emerald-700 dark:text-emerald-400">{total} contatos carregados</span>
                    </div>
                    <button 
                      onClick={handleRemoveDuplicates}
                      className="w-full py-2 bg-white dark:bg-slate-900 border border-emerald-200 dark:border-emerald-800 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 rounded-lg text-[10px] font-bold transition-colors flex items-center justify-center gap-2 shadow-sm"
                    >
                      <Eraser size={14}/> Remover Números Duplicados
                    </button>
                  </div>
                )}
              </div>

              {/* LOGS TABELA DE ENVIO E EXPORTAÇÃO (Painel agora é Dinâmico) */}
              <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-4 rounded-2xl overflow-hidden shadow-sm text-slate-900 dark:text-white transition-all">
                <div className="flex justify-between items-center mb-3">
                  <h3 className="font-bold flex items-center gap-2 text-sm text-slate-800 dark:text-white">
                    <ListIcon size={16} className="text-emerald-500 dark:text-emerald-400" /> 
                    Histórico de Envio
                  </h3>
                  <div className="flex gap-2">
                    <button onClick={exportToExcel} className="p-1.5 bg-slate-50 dark:bg-slate-800 hover:bg-emerald-500 dark:hover:bg-emerald-600 border border-slate-200 dark:border-slate-700 hover:border-emerald-500 rounded-lg text-slate-600 dark:text-slate-300 hover:text-white dark:hover:text-white transition-all text-[10px] font-bold flex items-center gap-1 shadow-sm" title="Exportar para Excel">
                      <Download size={12} /> Excel
                    </button>
                    <button onClick={exportToPDF} className="p-1.5 bg-slate-50 dark:bg-slate-800 hover:bg-rose-500 dark:hover:bg-rose-600 border border-slate-200 dark:border-slate-700 hover:border-rose-500 rounded-lg text-slate-600 dark:text-slate-300 hover:text-white dark:hover:text-white transition-all text-[10px] font-bold flex items-center gap-1 shadow-sm" title="Exportar para PDF">
                      <FileText size={12} /> PDF
                    </button>
                  </div>
                </div>
                
                <div className="grid grid-cols-2 p-2 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-100 dark:border-slate-700 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider rounded-t-lg">
                  <span>Número de Destino</span>
                  <span className="text-right">Status Final</span>
                </div>
                ...
                <div className="max-h-[350px] overflow-y-auto custom-scrollbar p-1 space-y-1 bg-white dark:bg-slate-900/50 rounded-b-lg">
                  {engine.logs.map((log, i) => (
                    <div key={i} className="flex flex-col p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/60 text-xs transition-colors border border-transparent hover:border-slate-100 dark:hover:border-slate-700">
                      <div className="grid grid-cols-2 items-center">
                        <span className="text-slate-600 dark:text-slate-300 font-mono tracking-tight">{log.number.split('@')[0]}</span>
                        <span className={cn(
                          "text-right font-bold text-[10px] uppercase px-2 py-0.5 rounded-full justify-self-end shadow-sm border",
                          log.status === 'success' ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-500/20" : 
                          log.status === 'skipped' ? "bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-100 dark:border-amber-500/20" : 
                  ...
                  {engine.logs.length === 0 && (
                    <div className="p-6 text-center text-slate-400 dark:text-slate-600 text-xs italic">
                      Nenhum registro de envio nesta sessão. Inicie os disparos para acompanhar a tabela.
                    </div>
                  )}
                </div>
              </div>
